...
function addToCart($product_id,$cartID){
    ...
    $check_sq="select * from cart_items where cart_id=$cartID and product_id=$product_id ";
    $check_op=doQuery($check_sq);
    if(mysqli_fetch_row($check_op)==0){
    $sql ="insert into cart_items (cart_id,product_id)values($cartID,$product_id)";
    $op=doQuery($sql);
    if($op){
...
function makeOrder(){
    ...
    #########################################
    // $sql1="select * from cart_items where cart_id=$cart_id";
    // $op1=doQuery($sql1);
    // $cartDetail_data=mysqli_fetch_assoc($op1);
   ...
   values ($number_orders,'$totalPrice',$ID,'$shippingDate')";
    ...
    // $sql="select orders.* ,users_type.type_name from users
    //  inner join orders on orders.customer_id=users.id 
    //  inner join users_type on users.userType_id=users_type.id; ";
}
function makeDetailOrder(){
    
    $orderID=makeOrder();
    ###############################################################
    for ($i=0; $i <$_SESSION['orderDetails'] ; $i++) { 
